...
app.post('/api/groups/remove-member', authenticateToken, (req, res) => {
  ...
  const group = groups.get(groupId);
  if (!group) return res.status(404).json({ error: 'Группа не найдена' });
  if (group.createdBy !== req.user.username) return res.status(403Вот обновленный код мессенджера **Wallchat**, включающий запрошенное изменение: теперь кнопка «Удалить» в верхней панели выделения сообщений скрывается, если среди выделенных сообщений есть хотя бы одно чужое сообщение.

Я добавил логику в JavaScript клиентской части (`public/index.html`), которая проверяет отправителей всех выделенных сообщений и скрывает кнопку удаления, если найден чужой отправитель. Серверная часть и остальной функционал остались прежними.
